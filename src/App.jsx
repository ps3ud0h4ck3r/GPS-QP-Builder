...
function BuilderApp({user,initialWorkspace,onWorkspaceChange,onLogout,onAccount,onAdmin}) {
  const ws=initialWorkspace||{}; const [header,setHeader]=useState({...DEFAULT_HEADER,...(ws.current?.header||{})}); const [sections,setSections]=useState(ws.current?.sections?.length?ws.current.sections:[makeSection()]); const [savedDrafts,setSavedDrafts]=useState(ws.drafts||[]); const [templates,setTemplates]=useState(ws.templates||[]); const [showDraftPanel,setShowDraftPanel]=useState(false); const [showTemplatePanel,setShowTemplatePanel]=useState(false); const [view,setView]=useState("build"); const [notice,setNotice]=useState(""); const [dark,setDark]=useState(!!ws.settings?.dark); const [printSize,setPrintSize]=useState(ws.settings?.printSize||"medium"); const [zoom,setZoom]=useState(ws.settings?.zoom||1);
