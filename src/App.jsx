...
function BuilderApp({user,initialWorkspace,onWorkspaceChange,onLogout,onAccount,onAdmin}){
  ...
  return <div className="app-shell">
    ...
    <header className="topbar"><div className="brand"><strong>GPS QP Builder</strong>{user?.username&&<span className="user-chip">{user.username}</span>}</div><div className="top-actions"><button className={view==="build"?"active":""} onClick={()=>setView("build")}>Build</button><button className={view==="preview"?"active":""} onClick={()=>setView("preview")}>Preview</button><button onClick={newPaper}>New Paper</button><button onClick={()=>setShowDraftPanel(d=>!d)}>Drafts {savedDrafts.length?`(${savedDrafts.length})`:""}</button><button onClick={()=>setDark(d=>!d)} title="Toggle dark mode">{dark?"☀":"◐"}</button>{user?.role==="admin"&&<button onClick={onAdmin}>Admin</button>}<button onClick={onAccount}>Account</button><button onClick={onLogout}>Logout</button></div></header>
